...
// ... (KEEP YOUR CHANGELOG ARRAY EXACTLY AS IS) ...
const CHANGELOG = [
  { id: "v2.5", version: "v2.5.0", date: "Jan 29, 2026", title: "System Hardening & UX Overhaul", icon: KeyRound, desc: "Real-time security ops, enhanced profile management, and global intel integration.", changes: ["Deployed Real-Time Password Recovery.", "Integrated Profile Picture Upload.", "Refactored Navbar with Wikipedia Search.", "Optimized Dashboard layout.", "Enhanced 'Forgot Password' flow."] },
  { id: "v2.4", version: "v2.4.0", date: "Jan 26, 2026", title: "Identity & Profile Ops", icon: User, desc: "User profile management with secure editing.", changes: ["Deployed 'Operative Profile' page.", "Implemented Secure Edit Mode.", "Added 'Live Preview'.", "Visual overhaul of Stats cards."] },
  { id: "v2.3", version: "v2.3.0", date: "Jan 26, 2026", title: "Tactical Dashboard 2.0", icon: LayoutGrid, desc: "New Topic Tracks & Compact Grid Layout.", changes: ["Replaced levels with 5 Tracks.", "Migrated to a 3-Column Grid.", "Redesigned Lesson Cards.", "Implemented dynamic filtering."] },
  // ... rest of your changelog ...
];
  ...
  return (
    <div className={`min-h-screen p-4 sm:p-8 overflow-x-hidden ${theme.bg} ${theme.text}`}>
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h1 className="text-3xl md:text-5xl font-black tracking-tighter mb-2 flex items-center gap-3">
        ...
        {/* CHRONOLOGICAL FLOW GRID (Centered on Mobile) */}
          ...
          {[...CHANGELOG].map((log, index, array) => (
            <React.Fragment key={log.id}>
              ...
              <motion.div 
                ...
                className={`relative w-full sm:w-72 h-44 p-6 rounded-[2rem] border cursor-pointer group transition-all duration-300 z-10 ${theme.card}`}
              ...
              <div className="fixed inset-0 flex items-center justify-center z-[70] pointer-events-none p-4">
                {CHANGELOG.filter(l => l.id === selectedId).map(log => (
                  <motion.div 
                    ...
                    className={`pointer-events-auto w-full max-w-xl p-6 md:p-8 rounded-[2.5rem] border shadow-2xl relative overflow-hidden ${theme.modal}`}
                  >
                    <button 
                    ...
                    <div className="flex items-center gap-5 mb-8">
                      ...
                      <div>
                        <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter">{log.title}</h2>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="px-3 py-1 rounded-lg bg-cyan-600 text-white text-[10px] font-black uppercase tracking-widest">{log.version}</span>
                          <span className={`text-xs font-bold flex items-center gap-1.5 ${theme.subText}`}><Calendar size={14} /> {log.date}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {log.changes.map((change, idx) => (
                        ...
                        >
                          ...
                          <p className={`text-sm font-medium leading-relaxed ${theme.subText}`}>{change}</p>
                        </motion.div>
                      ))}
                    </div>
                  </motion.div>
